<template>
    <div>
        ...
        <el-table :data="responseInfo.slice((currentPage-1)*pageSize, currentPage*pageSize)" style="width: 100%" :cell-class-name="cellClassName" :current-page.sync="currentPage">
        ...
        <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :page-sizes="[1, 2, 4]" :page-size="pageSize" layout="total, sizes, prev, pager, next, jumper" :total="responseInfo.length">
        </el-pagination>
    </div>
</template>
    ...
    methods: {
        ...
        fuzzySearch: function () {
            axios
                ...
                .then((out) => {
                    if (out.status == 200) {
                        this.responseInfo = out.data.requests;
                    } else {
                        alert("请刷新重试！");
                    }
                })
